...
import li.mofanx.epso.contentObserver
...
private val AccessibilityEvent.safeSource: AccessibilityNodeInfo?
    get() = if (className == null) {
        null // https://github.com/mofanx/epso/issues/426 event.clear 已被系统调用
    } else {
        try {
...
// https://github.com/mofanx/epso/issues/115
// https://github.com/mofanx/epso/issues/650
// 限制节点遍历的数量避免内存溢出
...
fun AccessibilityNodeInfo.isExpired(expiryMillis: Long): Boolean {
    ...
    if (generatedTime == -1L) {
        // https://github.com/mofanx/epso/issues/759
        return true
    }
